...
function TrustBadgesBar() {
  return (
    <section className="border-y border-carbon-700 bg-carbon-900 py-6 sm:py-8">
      <Container>
        <ScrollReveal>
          <div className="flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap sm:justify-center sm:overflow-visible sm:pb-0">
            {badges.map((badge) => (
              <div
                key={badge.id}
                className="hover-card flex h-20 w-[210px] sm:w-[220px] shrink-0 snap-start items-center gap-3 rounded-lg border border-carbon-600 bg-carbon-800 px-4 sm:px-5"
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-carbon-400 leading-tight mt-0.5">
                    {badge.subtitle}
                  </p>
                </div>
              </div>
