import Link from "next/link";
import { MapPin, Phone, Mail, Shield, ArrowRight } from "lucide-react";
import { siteSettings } from "@/data/siteSettings";

const iconClass = "h-5 w-5 fill-current";

function FacebookIcon() {
  return (
    <svg viewBox="0 0 24 24" className={iconClass}>
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  );
}

function InstagramIcon() {
  return (
    <svg viewBox="0 0 24 24" className={iconClass}>
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
    </svg>
  );
}

function YoutubeIcon() {
  return (
    <svg viewBox="0 0 24 24" className={iconClass}>
      <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
    </svg>
  );
}

function TikTokIcon() {
  return (
    <svg viewBox="0 0 24 24" className={iconClass}>
      <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
    </svg>
  );
}

function XIcon() {
  return (
    <svg viewBox="0 0 24 24" className={iconClass}>
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  );
}

const services = [
  { label: "New Boiler Installation", href: "/services/new-boiler-installation/" },
  { label: "Boiler Repair", href: "/services/boiler-repair/" },
  { label: "Annual Boiler Service", href: "/services/boiler-servicing/" },
  { label: "Boiler Prices", href: "/boilers/" },
  { label: "Free Quote", href: "/#quote" },
];

const areas = [
  { label: "G74 (North & Central)", href: "/areas-we-serve/g74/" },
  { label: "G75 (South & West)", href: "/areas-we-serve/g75/" },
  { label: "The Murray", href: "/areas-we-serve/the-murray/" },
  { label: "Stewartfield", href: "/areas-we-serve/stewartfield/" },
  { label: "Hairmyres", href: "/areas-we-serve/hairmyres/" },
];

export default function Footer() {
  return (
    <footer className="bg-carbon-950 border-t border-carbon-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-10 lg:gap-8">
          <div>
            <Link href="/" className="inline-block mb-4">
              <div className="leading-none">
                <span className="block text-[9px] uppercase tracking-[0.2em] text-carbon-300 font-medium">
                  The
                </span>
                <span className="block text-sm font-bold tracking-tight text-white border-b-2 border-mint-500 inline">
                  EAST KILBRIDE
                </span>
                <span className="block text-base font-extrabold tracking-tight text-white">
                  BOILER
                </span>
                <span className="block text-[9px] uppercase tracking-[0.2em] text-carbon-300 font-medium">
                  Company
                </span>
              </div>
            </Link>

            <div className="flex items-center gap-2 mb-4 mt-4">
              <div className="flex items-center justify-center w-10 h-10 bg-gold rounded-sm">
                <Shield className="h-6 w-6 text-carbon-900" />
              </div>
              <span className="text-sm font-semibold text-gold">Gas Safe</span>
            </div>

            <p className="text-xs uppercase tracking-wide text-white font-bold mb-2">
              East Kilbride&apos;s #1 Boiler Replacement Company
            </p>
            <p className="text-xs text-carbon-400 mb-4">
              Serving All Areas of East Kilbride G74/G75 Since 2017!
            </p>

            <div className="flex items-center gap-3">
              <a
                href={siteSettings.socialLinks.facebook}
                aria-label="Facebook"
                className="text-carbon-400 hover:text-mint-500 transition-colors"
              >
                <FacebookIcon />
              </a>
              <a
                href={siteSettings.socialLinks.instagram}
                aria-label="Instagram"
                className="text-carbon-400 hover:text-mint-500 transition-colors"
              >
                <InstagramIcon />
              </a>
              <a
                href={siteSettings.socialLinks.youtube}
                aria-label="YouTube"
                className="text-carbon-400 hover:text-mint-500 transition-colors"
              >
                <YoutubeIcon />
              </a>
              <a
                href={siteSettings.socialLinks.tiktok}
                aria-label="TikTok"
                className="text-carbon-400 hover:text-mint-500 transition-colors"
              >
                <TikTokIcon />
              </a>
              <a
                href={siteSettings.socialLinks.x}
                aria-label="X"
                className="text-carbon-400 hover:text-mint-500 transition-colors"
              >
                <XIcon />
              </a>
            </div>
          </div>

          <div>
            <h3 className="text-xs uppercase tracking-[0.2em] text-white font-bold mb-4">
              Our Services
            </h3>
            <ul className="space-y-2.5">
              {services.map((service) => (
                <li key={service.label}>
                  <Link
                    href={service.href}
                    className="text-sm text-carbon-400 hover:text-mint-500 transition-colors"
                  >
                    {service.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          <div>
            <h3 className="text-xs uppercase tracking-[0.2em] text-white font-bold mb-4">
              Areas We Cover
            </h3>
            <ul className="space-y-2.5">
              {areas.map((area) => (
                <li key={area.label}>
                  <Link
                    href={area.href}
                    className="text-sm text-carbon-400 hover:text-mint-500 transition-colors"
                  >
                    {area.label}
                  </Link>
                </li>
              ))}
              <li>
                <Link
                  href="/areas-we-serve/"
                  className="inline-flex items-center gap-1 text-sm text-mint-500 hover:text-mint-400 transition-colors font-medium"
                >
                  View All 35 Areas
                  <ArrowRight className="h-3 w-3" />
                </Link>
              </li>
            </ul>
          </div>

          <div>
            <h3 className="text-xs uppercase tracking-[0.2em] text-white font-bold mb-4">
              Get in Touch
            </h3>
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                <MapPin className="h-4 w-4 text-mint-500 mt-0.5 shrink-0" />
                <span className="text-sm text-carbon-400">
                  {siteSettings.address.street}, {siteSettings.address.city} -{" "}
                  {siteSettings.address.region}, {siteSettings.address.postcode}
                </span>
              </li>
              <li className="flex items-center gap-3">
                <Phone className="h-4 w-4 text-mint-500 shrink-0" />
                <a
                  href={siteSettings.phoneHref}
                  className="text-sm text-carbon-400 hover:text-mint-500 transition-colors"
                >
                  {siteSettings.phone}
                </a>
              </li>
              <li className="flex items-center gap-3">
                <Mail className="h-4 w-4 text-mint-500 shrink-0" />
                <a
                  href={`mailto:${siteSettings.email}`}
                  className="text-sm text-carbon-400 hover:text-mint-500 transition-colors break-all"
                >
                  {siteSettings.email}
                </a>
              </li>
            </ul>
            <div className="mt-6">
              <p className="text-sm text-mint-500 font-semibold">
                Open 24/7 365 days a year
              </p>
              <p className="text-xs text-carbon-400 mt-1">
                Emergency boiler installations available!
              </p>
            </div>
          </div>
        </div>
      </div>

      <div className="border-t border-carbon-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-carbon-400">
            <p>
              2026 The East Kilbride Boiler Company - East Kilbride&apos;s Gas Safe Experts - EST.2017
            </p>
            <div className="flex items-center gap-4">
              <Link href="/privacy/" className="hover:text-mint-500 transition-colors">
                Privacy
              </Link>
              <Link href="/terms/" className="hover:text-mint-500 transition-colors">
                Terms & Conditions
              </Link>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
