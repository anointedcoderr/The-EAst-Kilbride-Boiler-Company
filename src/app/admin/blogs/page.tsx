...
import { listBlogs } from "@/lib/cmsBlogs";
import { BlogsListView } from "./BlogsListView";

export const metadata: Metadata = {
  title: "Manage blog | EKBC Admin",
...
export default async function AdminBlogsPage() {
  const { rows, error } = await listBlogs({ limit: 500 });
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
        {error ? (
          <div className="flex items-start gap-2 rounded-2xl border border-amber-400/40 bg-amber-500/10 p-5 text-sm text-amber-100">
            ...
            <div>
              <p className="font-bold">Blog posts unavailable.</p>
              <p className="mt-1">{error}</p>
            </div>
          </div>
        ) : (
          <BlogsListView initial={rows} />
        )}
      </div>
    </div>
  );
